...
<archdesc <?php if ($resource->levelOfDescriptionId):?>level="<?php if (in_array(strtolower($levelOfDescription = $resource->getLevelOfDescription()->getName(array('culture' => 'en'))), $eadLevels)): ?><?php echo strtolower($levelOfDescription).'"' ?><?php else: ?><?php echo 'otherlevel" otherlevel="'.$levelOfDescription.'"' ?><?php endif; ?><?php endif; ?> relatedencoding="ISAD(G)v2">
  ...
  <?php foreach ($subjects as $subject): ?>
    <subject<?php if ($subject->getTerm()->code):?> authfilenumber="<?php echo $subject->getTerm()->code ?>"<?php endif; ?><?php if ($ead->subjectHasNonBlankSourceNotes($subject)):?> source="<?php foreach ($subject->getTerm()->getSourceNotes() as $note): ?><?php echo $note ?><?php endforeach; ?>"<?php endif; ?>><?php echo escape_dc(esc_specialchars($subject->getTerm())) ?></subject>
  <?php endforeach; ?>
  ...
  <?php foreach ($resource->getDescendants()->orderBy('lft') as $descendant): ?>
    ...
    ?>
      ...
      <?php foreach ($archivistsNotes as $note): ?><processinfo><p><?php echo escape_dc(esc_specialchars($note)) ?></p></processinfo>
      <?php endforeach; ?>
      <?php endif; ?>
