  <did>
    ...
    <?php endif; ?>
    <?php endforeach; ?>
    <unitid encodinganalog="3.1.1" <?php if (isset($repository)): ?><?php if ($countrycode = $repository->getCountryCode()): ?><?php echo 'countrycode="'.$countrycode.'" ' ?><?php endif;?><?php if ($repocode = $repository->getIdentifier()): ?><?php echo 'repositorycode="'.preg_replace('/\n/', '<lb/>', esc_specialchars($repocode)).'" ' ?><?php endif; ?><?php endif; ?>><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($$resourceVar->getIdentifier())) ?></unitid>
    <?php endif; ?>
    <?php if (0 < strlen($value = $$resourceVar->getPropertyByName('standardNumber')->__toString())): ?>
    <unitid type="standard"><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($value)) ?></unitid>
    <?php endif; ?>
    <?php foreach ($$resourceVar->getDates() as $date): ?>
    <unitdate <?php if ($date->typeId != QubitTerm::CREATION_ID): ?><?php if ($type = $date->getType()->__toString()): ?><?php echo 'datechar="'.strtolower($type).'" ' ?><?php endif; ?><?php endif; ?><?php if ($startdate = $date->getStartDate()): ?><?php echo 'normal="'?><?php echo Qubit::renderDate($startdate) ?><?php if (0 < strlen($enddate = $date->getEndDate())): ?><?php echo '/'?><?php echo Qubit::renderDate($enddate) ?><?php endif; ?><?php echo '"' ?><?php endif; ?> encodinganalog="3.1.3"><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars(Qubit::renderDateStartEnd($date->getDate(array('cultureFallback' => true)), $date->startDate, $date->endDate))) ?></unitdate>
    <?php endforeach; // dates ?>
    <?php if (0 < strlen($value = $$resourceVar->getExtentAndMedium(array('cultureFallback' => true)))): ?>
    <physdesc>
      <extent encodinganalog="3.1.5"><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($value)) ?></extent>
    </physdesc>
    <?php endif; ?>
    <?php if ($value = $$resourceVar->getRepository()): ?>
    <repository>
      <corpname><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($value->__toString())) ?></corpname>
      <?php if ($address = $value->getPrimaryContact()): ?>
      <address>
        <?php if (0 < strlen($addressline = $address->getStreetAddress())): ?>
        <addressline><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($addressline)) ?></addressline>
        <?php endif; ?>
        ...
        <addressline><?php echo __('Telephone: ').preg_replace('/\n/', '<lb/>', esc_specialchars($addressline)) ?></addressline>
        <?php endif; ?>
        <?php if (0 < strlen($addressline = $address->getFax())): ?>
        <addressline><?php echo __('Fax: ').preg_replace('/\n/', '<lb/>', esc_specialchars($addressline)) ?></addressline>
        <?php endif; ?>
        <?php if (0 < strlen($addressline = $address->getEmail())): ?>
        <addressline><?php echo __('Email: ').preg_replace('/\n/', '<lb/>', esc_specialchars($addressline)) ?></addressline>
        <?php endif; ?>
    ...
    <note type="sourcesDescription"><p><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($$resourceVar->sources)) ?></p></note>
    <?php endif; ?>
    <?php if (0 < count($notes = $$resourceVar->getNotesByType(array('noteTypeId' => QubitTerm::GENERAL_NOTE_ID)))): ?><?php foreach ($notes as $note): ?><note type="<?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($note->getType(array('cultureFallback' => true)))) ?>" encodinganalog="3.6.1"><p><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($note->getContent(array('cultureFallback' => true)))) ?></p></note><?php endforeach; ?><?php endif; ?>
    <?php if (0 < strlen($value = $$resourceVar->getPropertyByName('statementOfScaleCartographic')->__toString())): ?>
    <materialspec type='cartographic'><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($value)) ?></materialspec>
    <?php endif; ?>
    <?php if (0 < strlen($value = $$resourceVar->getPropertyByName('statementOfProjection')->__toString())): ?>
    <materialspec type='projection'><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($value)) ?></materialspec>
    <?php endif; ?>
    <?php if (0 < strlen($value = $$resourceVar->getPropertyByName('statementOfCoordinates')->__toString())): ?>
    <materialspec type='coordinates'><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($value)) ?></materialspec>
    <?php endif; ?>
    <?php if (0 < strlen($value = $$resourceVar->getPropertyByName('statementOfScaleArchitectural')->__toString())): ?>
    <materialspec type='architectural'><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($value)) ?></materialspec>
    <?php endif; ?>
    <?php if (0 < strlen($value = $$resourceVar->getPropertyByName('issuingJurisdictionAndDenomination')->__toString())): ?>
    <materialspec type='philatelic'><?php echo preg_replace('/\n/', '<lb/>', esc_specialchars($value)) ?></materialspec>
    <?php endif; ?>
